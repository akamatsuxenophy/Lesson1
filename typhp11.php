...
<html>
  ...
  <body>
    <?php
      ...
      class MyValidator {
        //エラーメッセージを格納するためのプライヴェート変数(配列)
        private $_errors;

        //コンストラクタ
        public function __construct($encoding = 'UTF-8') {
          //プライベート変数$ _errors を初期化
          $_error = [];
          //内部文字コードを設定
          mb_internal_encoding($encoding);
          // $_GET, $_POST, $_COOKIE　のエンコーディングをチェック
          $this->checkEncoding($_GET);
          $this->checkEncoding($_POST);
          $this->checkEncoding($_COOKIE);
          // $_GET, $_POST, $_COOKIE の nullバイトをチェック
          $this->checkNull($_GET);
    ...
    public function lengthCheck($value, $name, $len) {
      if(trim($value) !== '') {
        if(mb_strlen($value > $len)) {
          $this->_error[] = "{$name}は{$len}文字以内で入力してください。";
        }
      }
    }
    ...
    //プライベート変数 _errorにエラー情報が含まれる場合にはリスト表示
